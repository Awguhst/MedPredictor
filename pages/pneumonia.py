...
st.set_page_config(page_title="X_Ray_P", layout="centered")
...
def load_model(device):
    ...
    model.load_state_dict(torch.load('./models/pneumonia_weights.pth', map_location=device))
    model.to(device)
    model.eval()
    return model
...
st.markdown(
    ...
    Upload a chest X-ray image to predict whether the person has Pneumonia or not.
    </p>
    """,
    unsafe_allow_html=True,
)
...
EXAMPLE_IMAGE_PATH = os.path.join(EXAMPLES_FOLDER, "Not Pneumonia.jpeg")
